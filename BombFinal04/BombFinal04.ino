...
const byte s7sTWOAddress = 0x73; // define the I2C address of the S7S (default is 0x71)
...
char keypadONECode [4] = {'5', '3', '2', '8'}; // code to be unlocked
...
char keypadTWOCode [4] = {'3', '7', '2', '5'}; // code to be unlocked
...
int cableFailPin = 12;
int cablePin = 10;
int piezoONEPin = 11; // pin connected to the Piezos
int piezoTWOPin = 0; // CHANGE!!! ----- !!!
int openPin = 42;
int opening = 0;
int armed = 0;
int won = 2;
...
void setup() {
  ...
  pinMode(piezoONEPin, OUTPUT);
  pinMode(piezoTWOPin, OUTPUT);
...
void loop() {
  ...
  if ((keypadONEEntry[0] != keypadVoid) &&  (keypadONEEntry[1] != keypadVoid) &&  (keypadONEEntry[2] != keypadVoid) &&  (keypadONEEntry[3] != keypadVoid)) { // checks if all slots are filled with keypresses
    if ((keypadONEEntry[0] == keypadONECode[0]) &&  (keypadONEEntry[1] == keypadONECode[1]) &&  (keypadONEEntry[2] == keypadONECode[2]) &&  (keypadONEEntry[3] == keypadONECode[3]))   { // checks if keypresses match the code
      ...
      beep(100); seconds -= 20;
      colorWipe(strip.Color(0, 0, 0), 1); // Out
      beep(100); seconds -= 20;
      colorWipe(strip.Color(255, 0, 0), 1); // Red
      beep(100); seconds -= 20;
...
void beep(unsigned char delayms) {
  analogWrite(piezoONEPin, 250);      // Almost any value can be used except 0 and 255
