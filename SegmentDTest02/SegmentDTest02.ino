...
const byte s7sTWOAddress = 0x72;
const byte s7sTHREEAddress = 0x73;
char testCodeTWO [4] = {'1', '4', '3', '4'};
char testCodeTHREE [4] = {'4', '3', '2', '1'};
...
void setup()
{
  ...
  s7sSendStringI2C("-GO-", s7sTHREEAddress); // start & reset signal
  setDecimalsI2C(0b111111, s7sTHREEAddress);  // Turn on all decimals, colon, apos

  // Flash brightness values at the beginning
  setBrightnessI2C(0, s7sTWOAddress); // Lowest brightness
  setBrightnessI2C(0, s7sTHREEAddress); // Lowest brightness
  delay(1500);
  setBrightnessI2C(255, s7sTWOAddress);  // High brightness
  setBrightnessI2C(255, s7sTHREEAddress);  // High brightness
  delay(1500);

  clearDisplayI2C(s7sTWOAddress); // Clear the display before jumping into loop
  clearDisplayI2C(s7sTHREEAddress);
}

void loop()
{
  s7sSendStringI2C(testCodeTWO, s7sTWOAddress); // sends characters to the display
  s7sSendStringI2C(testCodeTHREE, s7sTHREEAddress); // sends characters to the display
